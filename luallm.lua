...
local function ensure_notes_dir()
    exec("mkdir -p " .. sh_quote(NOTES_DIR))
end

local function get_notes_path(model_name)
    return NOTES_DIR .. "/" .. model_name .. ".md"
end

local function has_notes(model_name)
    return file_exists(get_notes_path(model_name))
end
...
local function add_note(model_name, note_text)
    ...
    -- Find ## Notes section and insert before ## Summary
    local timestamp = os.date("%Y-%m-%d %H:%M")
    local new_note = string.format("- %s  %s\n", timestamp, note_text)
    
    -- Find position to insert (before ## Summary or at end of ## Notes section)
...
local function list_models_with_notes(config)
    ensure_notes_dir()
    local models = {}
    ...
    -- Sort by notes modification time, newest first
...
local function main(args)
    ...
    elseif args[1] == "notes" then
        ...
        if not subcommand then
            -- Interactive picker to select model and show notes
            local all_models = list_models(config.models_dir)
            ...
            local selected = show_picker(model_objs, config, "Select a model to view notes (↑/↓ arrows, Enter to confirm, q to quit):")
            if not selected then
                os.exit(0)
            end
            
            -- Show notes for selected model
        ...
        elseif subcommand == "list" then
            ...
            local models = list_models_with_notes(config)
            ...
            -- Calculate column widths and print
            local max_name, max_size, max_quant = calculate_column_widths(models)
            for _, m in ipairs(models) do
                print("  " .. format_model_row(m, max_name, max_size, max_quant))
            end
            
        elseif subcommand == "path" then
            -- Show notes path for a model
            if #args < 3 then
                print("Error: Missing model name")
        ...
        elseif subcommand == "add" then
            ...
            elseif #matches == 1 then
                ...
                local selected = show_picker(match_models, config, "Select a model to add note (↑/↓ arrows, Enter to confirm, q to quit):")
                if selected then
        ...
        elseif subcommand == "edit" then
            ...
            elseif #matches == 1 then
                ...
                local cmd = editor .. " " .. sh_quote(notes_path)
                os.execute(cmd)
            else
                local match_models = {}
                for _, name in ipairs(matches) do
                ...
                local selected = show_picker(match_models, config, "Select a model to edit notes (↑/↓ arrows, Enter to confirm, q to quit):")
                if selected then
            ...
            elseif #matches == 1 then
                ...
                local selected = show_picker(match_models, config, "Select a model to view notes (↑/↓ arrows, Enter to confirm, q to quit):")
                if selected then
                    local content = read_notes(selected)
                    if content then
                        print(content)
